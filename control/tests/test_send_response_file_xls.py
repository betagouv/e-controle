...
class SendResponseFileXLSRunner:
...
def test_download_response_file_xls_works_if_the_control_is_associated_with_the_user(client):
    questionnaire = factories.QuestionnaireFactory(is_draft=False)
    user = utils.make_audited_user(questionnaire.control)
    utils.login(client, user=user)
    url = reverse('send-response-file-list', args=[questionnaire.id])
...
def test_download_response_file_xls_fails_if_the_control_is_not_associated_with_the_user(client):
    questionnaire = factories.QuestionnaireFactory(is_draft=False)
