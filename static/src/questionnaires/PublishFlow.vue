<template>
  <modal-flow ref="modalFlow" :action-function="publishFunction">
    ...
    <template v-slot:error-message>
      <div>Le questionnaire n'a pas pu être publié. Vous pouvez réessayer.</div>
      <div>
    ...
    <template v-slot:success-modal-body>
      <div class="modal-header border-bottom-0 flex-column align-items-center">
        ...
        <h4 class="text-center">Bravo, votre questionnaire est publié!</h4>
        <div class="mt-5">
          <p>Pensez à informer l'organisme contrôlé.</p>
          <p>
            Si des réponses sont déposées par l'organisme interrogé, vous
            recevrez un email de notification dès le lendemain 8 heures.
          </p>
        </div>
      </div>
      <div class="modal-body text-center">
        <div class="mt-5 flex-row justify-content-center">
          <button type="button" class="btn btn-primary ml-2" @click="goHome">
            <i class="fa fa-chevron-left mr-2"></i>
            Revenir à l'accueil
          </button>
          <a
...
export default Vue.extend({
  ...
  computed: {
    ...
    emailHeader: function () {
      ...
      const currentControl = this.controls.find(
        (control) => control.id === this.questionnaire.control
      );

      if (currentControl) {
        const editors = uniq(
          currentControl.questionnaires.map((q) => q.editor.email)
        ).join(";");
        const audited = this.users
          .filter((u) => u.profile_type === "audited")
          .map((u) => u.email)
          .join(";");
  ...
  methods: {
    getUsers() {
      axios.get(backend.getUsersInControl(this.controlId)).then((response) => {
